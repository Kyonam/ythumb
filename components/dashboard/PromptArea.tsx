...
>(({ className, sideOffset = 4, showArrow = false, ...props }, ref) => (
  <TooltipPrimitive.Portal>
    <TooltipPrimitive.Content
      ...
      className={cn(
        "relative z-50 max-w-[280px] rounded-md bg-zinc-900 text-white px-3 py-1.5 text-xs animate-in fade-in-0 zoom-in-95 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 shadow-xl border border-zinc-800",
        className
      )}
      {...props}
    >
...
>(({ className, align = "center", sideOffset = 4, ...props }, ref) => (
  <PopoverPrimitive.Portal>
    <PopoverPrimitive.Content
      ...
      sideOffset={sideOffset}
      className={cn(
        "z-50 w-64 rounded-2xl bg-zinc-900 p-2 text-white shadow-2xl border border-zinc-800 outline-none animate-in data-[state=open]:fade-in-0 data-[state=closed]:fade-out-0 data-[state=open]:zoom-in-95 data-[state=closed]:zoom-out-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2",
        className
      )}
      {...props}
    />
...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Overlay
    ref={ref}
    className={cn(
      "fixed inset-0 z-[60] bg-black/80 backdrop-blur-md data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0",
      className
    )}
    {...props}
  />
...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ...
      {...props}
    >
      <div className="relative bg-zinc-900 rounded-[28px] overflow-hidden shadow-2xl p-1 border border-zinc-800">
        {children}
        <DialogPrimitive.Close className="absolute right-3 top-3 z-10 rounded-full bg-zinc-800 p-2 hover:bg-zinc-700 transition-all text-zinc-400 hover:text-white">
...
export const PromptArea = React.forwardRef<HTMLTextAreaElement, React.TextareaHTMLAttributes<HTMLTextAreaElement>>(
  ({ className, ...props }, ref) => {
    ...
    return (
      <div className={cn(
        "relative overflow-hidden flex flex-col rounded-[32px] p-3 shadow-2xl transition-all bg-zinc-900 border border-zinc-800/50 focus-within:border-zinc-700 cursor-text w-full max-w-3xl",
        className
      )}>
        <BorderBeam
          duration={8}
        ...
        {imagePreview && (
          <Dialog open={isImageDialogOpen} onOpenChange={setIsImageDialogOpen}>
            <div className="relative mb-2 w-fit rounded-2xl p-1 bg-zinc-800/30">
              <button type="button" className="transition-transform hover:scale-[1.02]" onClick={() => setIsImageDialogOpen(true)}>
                <img src={imagePreview} alt="Image preview" className="h-16 w-16 rounded-xl object-cover" />
              </button>
              <button
                onClick={handleRemoveImage}
                className="absolute -right-2 -top-2 z-10 flex h-5 w-5 items-center justify-center rounded-full bg-zinc-800 text-white border border-zinc-700 hover:bg-zinc-700 transition-colors"
            ...
            <DialogContent>
              <img src={imagePreview} alt="Full size preview" className="w-full max-h-[85vh] object-contain rounded-[24px]" />
            </DialogContent>
          </Dialog>
        )}
        ...
        <textarea
          ...
          className="w-full resize-none border-0 bg-transparent p-3 text-lg text-white placeholder:text-zinc-500 focus:ring-0 focus-visible:outline-none min-h-[60px] custom-scrollbar"
          {...props}
        />
            ...
            <div className="flex items-center gap-2">
              <Tooltip>
                <TooltipTrigger asChild>
                  <button type="button" onClick={handlePlusClick} className="flex h-10 w-10 items-center justify-center rounded-full text-zinc-400 transition-colors hover:bg-zinc-800 hover:text-white focus-visible:outline-none">
                    <PlusIcon className="h-6 w-6" />
                    <span className="sr-only">Attach image</span>
                  </button>
                </TooltipTrigger>
                <TooltipContent side="top" showArrow={true}><p>Attach visual reference</p></TooltipContent>
              </Tooltip>
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <PopoverTrigger asChild>
                      <button type="button" className="flex h-10 items-center gap-2 rounded-full px-4 text-sm font-medium text-zinc-400 transition-colors hover:bg-zinc-800 hover:text-white focus-visible:outline-none">
                        <Settings2Icon className="h-4 w-4" />
                        {!selectedTool && 'Models'}
                      </button>
                    </PopoverTrigger>
                  </TooltipTrigger>
                  <TooltipContent side="top" showArrow={true}><p>Choose AI Model</p></TooltipContent>
                </Tooltip>
                  ...
                  <div className="flex flex-col gap-1">
                    {toolsList.map(tool => (
                      <button
                        ...
                        className="flex w-full items-center gap-3 rounded-xl p-3 text-left text-sm hover:bg-zinc-800 transition-colors"
                      >
                        <tool.icon className="h-4 w-4 text-zinc-400" />
                        <span className="font-medium">{tool.name}</span>
                        {tool.extra && <span className="ml-auto text-[10px] font-bold uppercase tracking-wider text-zinc-500">{tool.extra}</span>}
                      </button>
                    ))}
                  </div>
                </PopoverContent>
              ...
              {activeTool && (
                <>
                  ...
                  <button onClick={() => setSelectedTool(null)} className="flex h-10 items-center gap-2 rounded-full px-4 text-sm font-semibold bg-blue-500/10 text-blue-400 border border-blue-500/20 hover:bg-blue-500/20 transition-all">
                    {ActiveToolIcon && <ActiveToolIcon className="h-4 w-4" />}
                    {activeTool.shortName}
                    <XIcon className="h-3 w-3 ml-1 opacity-60" />
                  </button>
                </>
              )}
              ...
              <div className="ml-auto flex items-center gap-2">
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button type="button" className="flex h-10 w-10 items-center justify-center rounded-full text-zinc-400 transition-colors hover:bg-zinc-800 hover:text-white focus-visible:outline-none">
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button
                      type="submit"
                      disabled={!hasValue}
                      className="flex h-10 w-10 items-center justify-center rounded-full text-sm font-medium transition-all focus-visible:outline-none disabled:opacity-30 bg-white text-black hover:bg-zinc-200 disabled:hover:bg-white"
                    >
                      <SendIcon className="h-6 w-6" />
                      <span className="sr-only">Generate</span>
                    </button>
                  </TooltipTrigger>
                  <TooltipContent side="top" showArrow={true}><p>Generate Thumbnail</p></TooltipContent>
                </Tooltip>
              </div>
            </div>
          </TooltipProvider>
